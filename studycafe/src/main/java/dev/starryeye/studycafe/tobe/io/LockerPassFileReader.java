...
public class LockerPassFileReader implements StudyCafeLockerPassProvider {
    ...
    private static final String LOCKER_PASSES_CSV_PATH = "src/main/resources/dev/starryeye/studycafe/locker.csv";

    @Override
    public StudyCafeLockerPasses getLockerPasses() {
        try {
            List<String> lines = Files.readAllLines(Paths.get(LOCKER_PASSES_CSV_PATH));
